function NND = nnd(vCat,Par,options)
%
...
%
    ...
    NND.vEta    = Inf(nEq,1);   % nearest-neighbour proximity
    NND.vT      = Inf(nEq,1);   % rescaled time
    NND.vR      = Inf(nEq,1);   % rescaled distance
    ...
    NND.vClust  = zeros(nEq,1); % 0 - background; 1 - clustered
    ...
    for j = 2:nEq
        ...
        NND.vEta(j)    = vEta;
        NND.vT(j)      = mT_ij(pind,j);
        NND.vR(j)      = mR_ij(pind,j);
