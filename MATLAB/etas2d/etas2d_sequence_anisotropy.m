function etas2d_sequence_anisotropy(Parent,beta1,vPar,fMmin,fM0,fTmax,nNmax,sPointProc,vBndr,vX,vY,mAnistroProb)
%
...
%
    ...
    for n = 1:nProd
        ...
        if fAsTime <= fTmax && nEqNum <= nNmax
            ...
            prob = interp2(vX,vY,mAnistroProb,lon,lat);
            %while lon > vBndr(1) && lon < vBndr(2) && lat > vBndr(3) && lat < vBndr(4) && rand() > mBgrndProb(ny,nx)
            while lon > vBndr(1) && lon < vBndr(2) && lat > vBndr(3) && lat < vBndr(4) && rand() > prob
                [lon, lat] = dr_rad_inverse(Parent.lon,Parent.lat,d,vPar(7)); % (Px,Py,d,q) power-law distributed random position from the parent event epicentre
                prob = interp2(vX,vY,mAnistroProb,lon,lat);
            end
            if lon > vBndr(1) && lon < vBndr(2) && lat > vBndr(3) && lat < vBndr(4) % ony accept events in vBound box
                ...
                if fMag > fM0
                    ...
                    etas2d_sequence_anisotropy(Parent_new,beta1,vPar,fMmin,fM0,fTmax,nNmax,sPointProc,vBndr,vX,vY,mAnistroProb);
                end
            end
        end
    end
