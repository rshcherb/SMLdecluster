function res = etas2d8p_spat_prod(vRk,vRk2,vDk,nJe,inR,d,q,vexpgam)
%
%   Computes the spatial integral for the productivity part of the likelihood function by considering the
...
%
    res    = zeros(nJe,1);
    f12pi  = 1.0/(2*pi);
    f1d2ex = 1.0./(d^2.*vexpgam);
    q1     = 1.0 - q;
    for n = 1:nJe
        if inR(n)      % for events inside the target region R
            res(n) = f12pi*sum(vDk(n,:).*(1.0 - (1.0 + vRk(n,:).^2.*f1d2ex(n)).^q1));
        else
            resR   = f12pi*sum(vDk(n,:).*(1.0 - (1.0 +  vRk(n,:).^2.*f1d2ex(n)).^q1));
            resR2  = f12pi*sum(vDk(n,:).*(1.0 - (1.0 + vRk2(n,:).^2.*f1d2ex(n)).^q1));
            res(n) = resR2 - resR;
        end
    end
end
