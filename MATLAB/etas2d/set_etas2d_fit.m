function ETAS = set_etas2d_fit(vCat,Model,mIP,varargin)
%
%   Author: Dr. Robert Shcherbakov, e-mail: rshcherb@uwo.ca
%
%   version: 1.0.0, 6 May 2021
%   ...
%   version: 1.0.1, 29 December 2021
%
    sSolver = 'fmincon'; % optimization solver
    nNk     = 1000;        % the number of radial segments for the spatial integrals
    for k = 1:length(varargin)
        if strcmp('Solver',varargin{k})
            sSolver = varargin{k+1};
        end
        if strcmp('NumSegments',varargin{k})
            nNk = varargin{k+1};
        end
    end
    %   T0 ------- Ts ---------------- Te ----- T1
...
%     ETAS.fUmax    = Model.fUmax;
    ...
    ETAS.sSolver  = sSolver;       % optimization method
    ...
    [vRk, vRk2, vDk, vReg_ref, nVer_ref] = segment_rad_angle(Model.vReg_targ,inR,[vCat(:,3),vCat(:,2)],nNk);
    ETAS.nNk      = nVer_ref;
