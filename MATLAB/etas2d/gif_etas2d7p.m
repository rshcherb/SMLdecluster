function vRate = gif_etas2d7p(vT,fX,fY,fU,vPar,d2,fFac,vCat)
%
%   Conditional ground intensity rate for the spatial ETAS model with 7 parameters
%
...
%
    ...
    vRate = zeros(1,nT);
    for jt = 1:nT
        nJ = find(vCat(:,1) < vT(jt),1,'last'); % find the last index for which vCat(:,1) < vT(j)
        vRate(jt) = vPar(1)*fU + ...
                  fFac*sum( exp(vPar(3).*vCat(1:nJ,4)).*...
                          (((vT(jt) - vCat(1:nJ,1))./vPar(4) + 1).^(-vPar(5))).*...
                          ((1 + ( (fX - vCat(1:nJ,3)).^2 + (fY - vCat(1:nJ,2)).^2 )/d2).^(-vPar(7))) ); % \mu + sum
    end
end
