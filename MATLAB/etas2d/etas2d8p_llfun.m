function fLle = etas2d8p_llfun(vPar,vCat,ETAS)
%
...
%
    vexpalfgam = exp((vPar(3)-vPar(8)).*vCat(1:ETAS.nJe,4)); % assumes that m_0 is subtracted
    vexpgam = exp(vPar(8).*vCat(1:ETAS.nJe,4));
    ...
    for j = (ETAS.linTR')         % the earthquakes during the target time interval t_j \in [Ts, Te] and inside the target region (x_j,y_j) \in R
        s = vPar(1)*ETAS.vU(j) + fFac*sum( vexpalfgam(1:(j-1)).*((1 + vtc(j) - vtc(1:(j-1))).^(-vPar(5)) ).*...
            ( (1 + ( (vx(j) - vx(1:(j-1))).^2 + (vy(j) - vy(1:(j-1))).^2 )./(d2.*vexpgam(1:(j-1)))).^(-vPar(7)) ) ); % \mu + sum
        if s > 1e-25
    ...
    fLle = -etas2d8p_prod(vPar,vCat,ETAS) + sumlog_gif; %
    %disp(vPar)
    %disp(fLle)
end
